    ...
    def download_csvs(self):
        ...
        for timespan in timespans:
            ...
            if os.path.isfile(filepath):
                ...
                df = polygon_api.get_prices(self.ticker, timespan, self.start_date, self.end_date)
                df.to_csv(f"data/raw/{self.ticker}_prices_{timespan}_{self.start_date}_{self.end_date}.csv", index=False)
                print(f"Success: data/raw/{self.ticker}_prices_{timespan}_{self.start_date}_{self.end_date}.csv!\n")
        ...
        if os.path.isfile(filepath):
            ...
            df = polygon_api.get_news(self.ticker, self.start_date, self.end_date)
            df.to_csv(f"data/raw/{self.ticker}_news_{self.start_date}_{self.end_date}.csv", index=False)
            print(f"Success: data/raw/{self.ticker}_news_{self.start_date}_{self.end_date}.csv!\n")
        self.files.append(filepath)
